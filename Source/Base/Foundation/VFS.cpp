...
static std::string Normalize(std::string path)
{
    ...
    auto it = std::unique(path.begin(), path.end(), [](char a, char b) { return a == '/' && b == '/'; });
...
static bool FindMount(const std::string& path, std::string& outVirtual, std::string& outPhysical)
{
    ...
    std::sort(sorted.begin(), sorted.end(), [](auto& a, auto& b) { return a.first.length() > b.first.length(); });

    for (const auto& m : sorted)
    {
        if (path.length() >= m.first.length() && path.compare(0, m.first.length(), m.first) == 0
...
static std::string Resolve(const std::string& path)
{
    ...
    {
        std::string rest = (norm.length() > virt.length()) ? norm.substr(virt.length() + 1) : "";
...
FS::File* Open(const std::string& path, FS::FileMode mode) { return FS::File::Open(Resolve(path), mode); }

std::string GetRealPath(const std::string& virtualPath) { return Resolve(virtualPath); }

bool IsFile(const std::string& path) { return FS::IsFile(Resolve(path)); }

bool IsDirectory(const std::string& path) { return FS::IsDirectory(Resolve(path)); }
...
void DuplicateFile(const std::string& from, const std::string& to) { FS::DuplicateFile(Resolve(from), Resolve(to)); }
...
std::string ParentPath(const std::string& path)
{
    ...
    {
        if (parent == phys
            || (parent.length() > phys.length() && parent.compare(0, phys.length(), phys) == 0
                && parent[phys.length()] == '/'))
        {
...
std::string FileName(const std::string& path) { return FS::Path::FullFileName(Resolve(path)); }

std::string FullFileName(const std::string& path) { return FS::Path::FullFileName(Resolve(path)); }

std::string Extension(const std::string& path) { return FS::Path::Extension(Resolve(path)); }

std::string Join(const std::string& a, const std::string& b) { return Normalize(a + "/" + b); }
